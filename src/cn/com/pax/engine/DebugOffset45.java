 ...
 *
 * @author luohl
 * @create 2017-12-20-11:08
 */
public class DebugOffset45 extends BaseEngine implements  BaseMoveWork {
    private String sendC4Str = null ;
...
//        serialConnection = SerialOperation.getInstance().getConnection();
        ...
        if (debugInfo.getRotateType() == null || debugInfo.getRotateAngle() == null){
            return 6;
        }
        List<String>strings;
        String pc2PhoneRev = null ;
        RecordLog.Logger().info("【调试偏转角度（TSPZ)】");
        if (pc2Phone!=null && pc2C4!=null && serialConnection!=null){
...
//                            logWriter.append("机械臂移动到下一个位置:"+MainFrame.jingShenJtf.getText()+"mm,并"+debugInfo.getRotateType()+debugInfo.getRotateAngle()+"度");
//                            UIUtils.updateTxA(recordTa,"机械臂移动到下一个位置:"+MainFrame.jingShenJtf.getText()+"mm,并"+debugInfo.getRotateType()+debugInfo.getRotateAngle()+"度"+"\r\n",true);
                            UIUtils.logColorUpdate(logWriter, "机械臂移动到下一个位置:"+MainFrame.jingShenJtf.getText()+"mm,并"+debugInfo.getRotateType()+debugInfo.getRotateAngle()+"度", recordTa,ColorUtils.BLACK);

                            if (Constant.LEFT.equals(debugInfo.getRotateType())||Constant.RITHT.equals(debugInfo.getRotateType()))
                                sendC4Str = Constant.RLROLL + debugInfo.getRotateAngle() +" "+MainFrame.jingShenJtf.getText().trim();
                            else{
                                sendC4Str = Constant.UDROLL + debugInfo.getRotateAngle() +" "+MainFrame.jingShenJtf.getText().trim();
                            }
                            pc2C4Rev = pc2C4.writeReadOneLine(sendC4Str);
                            if (pc2C4Rev.contains("ok")){
//                                logWriter.append("机械臂"+debugInfo.getRotateType()+debugInfo.getRotateAngle()+"度就位");
//                                UIUtils.updateTxA(recordTa,"机械臂"+debugInfo.getRotateType()+debugInfo.getRotateAngle()+"度就位"+"\r\n",true);
                                UIUtils.logColorUpdate(logWriter, "机械臂"+debugInfo.getRotateType()+debugInfo.getRotateAngle()+"度就位", recordTa, ColorUtils.BLACK);
                                ...
                                if (ret != 0){
                                    xml.addResult(caseTxt, "FAIL", logWriter.getLogFileName().substring(logWriter.getLogFileName().lastIndexOf("\\")+1),debugInfo.getRotateType()+debugInfo.getRotateAngle());
                                    xml.writeXml();
                                }else{
...
//                                    ",显示条码失败"+"\r\n",true);
                            UIUtils.logColorUpdate(logWriter, "PC发送给Phone的请求:"+ (FileName==null?"generate code":Constant.SHOW + FileName)+
                                    ",PC收到Phone的【错误】数据包:" + pc2PhoneRev + ",显示条码失败", recordTa,ColorUtils.RED);
                            return 1;
                        }
                    }else{
                        RecordLog.Logger().error("PC发送给C4的命令:" + sendC4Str + "," + "PC接受C4错误返回码:" + pc2C4Rev);
